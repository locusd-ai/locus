...
- [x] Create `crates/biem-registry/src/memory.rs`
...
- [x] Create `crates/biem-registry/src/memory_tests.rs` (or `#[cfg(test)]` module)
...
- **Validate**: `cargo test -p biem-registry` ✅ (18 tests)

### Step 3: Add DuckDB dependency ✅
- [x] Add `duckdb` crate to workspace `Cargo.toml` and `biem-registry/Cargo.toml`
- [x] Add `tracing` dependency for structured logging
- **Validate**: `cargo check -p biem-registry` ✅

### Step 4: DuckDB schema & migrations ✅
- [x] Create `crates/biem-registry/src/duckdb.rs`
...
- **Validate**: `cargo test -p biem-registry` ✅ (14 tests)

### Step 8: Review against contracts ✅
- [x] Compare `Registry` impl against trait in `biem-core/src/registry.rs` — all 13 methods covered
