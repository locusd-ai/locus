...
- [ ] Add `serde_json` to biem-parser for frontmatter value conversion
- [ ] Wire up in `biem-parser/Cargo.toml`
- **Validate**: `cargo check -p biem-parser`

### Step 2: Frontmatter extraction
- [ ] Create `crates/biem-parser/src/markdown.rs`
...
- **Validate**: `cargo test -p biem-parser`
...
- [ ] Write integration tests in `crates/biem-parser/tests/` using fixtures
- **Validate**: `cargo test -p biem-parser`

### Step 14: Review against contracts
- [ ] Compare `MarkdownParser` against `Parser` trait in `biem-core/src/parser.rs`
