...
## Goal
`bulk_index` should be safe to re-run against an already-indexed vault. Currently it fails with "path already registered" because `insert_doc` rejects duplicate paths. This affects both `locus index` (re-running) and `locusd` (restarting).
...
- Partially indexed vaults (interrupted `locus init`)
- Re-indexing after adding new files (without using the watcher)
- `locus index` run twice on the same vault
...
- **Validate**: delete a fixture file, re-run `locus index`, verify tombstoned ✅
...
- **Validate**: `cargo test -p locus-ingest` ✅ (15 tests pass)

### Step 5: Remove daemon skip-if-populated guard
- [x] Remove the `total_documents > 0` check in `locusd` — no longer needed since bulk_index is idempotent
