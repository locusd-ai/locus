//! Integration tests for .locusignore and default ignore patterns.
...
mod tests {
    ...
    #[test]
    fn test_locusignore_file_excludes_patterns() {
        let dir = tempfile::tempdir().unwrap();

        // Create .locusignore
        fs::write(dir.path().join(".locusignore"), "drafts/\n*.tmp.md\n").unwrap();
    ...
    fn test_incremental_event_in_ignored_dir_is_skipped() {
        ...
        assert_eq!(result.action, locus_ingest::IngestAction::Skipped);
    }
}
